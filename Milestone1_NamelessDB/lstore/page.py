...
class Page:
    ...
    def write(self, value):
        ...
        self.inc_num_records()

    def write_to_pos(self, value, position):
        if self.get_num_records() == 0:
            raise Exception("page.read_bytes: No data to read in this page")
        if position >= MAX_RECORDS or position < 0:
            raise Exception("page.read_bytes: Index out of range")

        if position > self.get_num_records() - 1:
            raise Exception("page.read_bytes: Index beyond number of written records in page")
    ...
    def read_int(self, index):
        return int.from_bytes(self.read_bytes(index), "big")
